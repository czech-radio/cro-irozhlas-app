...
def analyze_texts(
    texts: Iterable[Dict[str, str]], key: str
) -> "Generator[AnalysisResult, None]":
    """
    Return the analyzed texts.
    """
    url = "https://api.geneea.com/v3/analysis/T:CRo-transcripts"

    headers = {"Content-type": "application/json", "Authorization": f"user_key {key}"}
...
def main():
    try:
        ...
        results = analyze_texts(texts=load_texts(), key=GENEEA_PRIVATE_KEY)
